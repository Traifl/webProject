...
<template>
    <div v-if="global.showNewFolder" class="fixed inset-0 flex items-center justify-center z-50" @click.self="close">
        <div class="bg-white rounded-lg p-4 min-w-[300px] max-w-[500px] shadow-xl">
            ...
            <form @submit.prevent="handleSubmit" class="flex flex-col gap-0.5 text-gray-700">
                <div>
                    ...
                    <input v-model="name" type="text" class="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring" required placeholder="Enter folder name"/>
                </div>
